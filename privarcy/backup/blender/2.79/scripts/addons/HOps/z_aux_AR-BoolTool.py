bl_info = {
    "name": "BoolTool 0.2.3(BME)",
    "author": "Vitor Balbio",
    "version": (0, 2, 3),
    "blender": (2, 76, 0),
    "location": "View3D > Object > BoolTool",
    "description": "Improve de Boolean Usability... even more . And a little more past that.",
    "warning": "",
...
def Operation(context,_operation):
    user_preferences = bpy.context.user_preferences
    addon_pref = user_preferences.addons[__name__].preferences
    ...
    for selObj in bpy.context.selected_objects:
        if selObj != context.active_object and (selObj.type == "MESH" or selObj.type == "CURVE"):
            ...
            #Hard Ops Addition
            if addon_pref.use_bmesh:
                try:
                    newMod.use_bmesh = True
                except:
                    newMod.use_bmesh = False
                    print("Bmesh Is Enabled! / But it is not present on this boolean")
                    print("Bmesh Is Enabled! / This isn't a BMesh build of Blender")
...
class BoolTool_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        user_preferences = bpy.context.user_preferences
        addon_pref = user_preferences.addons[__name__].preferences
        self.layout.prop(self, "use_bmesh")
...
class BoolTool_Pref(AddonPreferences):
    ...
    #Hard Ops Addition
    use_bmesh = bpy.props.BoolProperty(name="Use Bmesh", default=False, update=UpdateBoolTool_Pref, description="Use The New Bmesh Boolean")
    
    use_wire = bpy.props.BoolProperty(name="Use Bmesh", default=False, update=UpdateBoolTool_Pref, description="Use The Wireframe Instead Of Boolean")
    
    def draw(self, context):
        layout = self.layout
        layout.label(text="Experimental Features:")
        layout.prop(self, "fast_transform", text = "Fast Transformation")
        layout.label(text="")
        layout.label(text="Hard Ops Additions")
