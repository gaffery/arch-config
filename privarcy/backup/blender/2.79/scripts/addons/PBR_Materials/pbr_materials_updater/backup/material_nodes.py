...
def append_material_node(self, context):
    ...
    if node_name=="Dielectric":
        ...
        princi.inputs[5].default_value = (0.5)
        princi.inputs[7].default_value = (0.55)
    elif node_name=="Atmosphere":
        volume = active_mat.node_tree.nodes.new("ShaderNodeVolumeScatter")
        volume.location = bpy.context.space_data.edit_tree.view_center
        volume.inputs[0].default_value = (0.224, 0.454, 0.95, 1)
        volume.inputs[2].default_value = (0.25)
        ...
        emission.location = bpy.context.space_data.edit_tree.view_center
        blackbody = active_mat.node_tree.nodes.new("ShaderNodeBlackbody")
        blackbody.location.x = emission.location.x-200
        blackbody.location.y = emission.location.y
