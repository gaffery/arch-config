    ...
    def draw(self, context):
        ...
        if pro_mode_enabled():
            if context.active_object.type == 'CAMERA':
                ...
                col.label("Camera Options")
                col.prop(cam, "lock_camera", text = "")
        ...
        if bpy.context.object and bpy.context.object.type == 'MESH':
            layout.menu("object.material_list_menu", icon_value=get_icon_id("Noicon"))
...
class arSub(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        if pro_mode_enabled():
            if is_multiselected == False:
                layout.operator_context = 'INVOKE_DEFAULT'
                layout.operator("sstep.objects", text = "(S) Step", icon_value=get_icon_id("Sstep"))

                layout.operator_context = 'INVOKE_DEFAULT'
                layout.operator("cstep.objects", text = "(C) Step", icon_value=get_icon_id("Cstep"))
...
class Meshtools(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        if pro_mode_enabled():
                ...
                #layout.operator("view3d.mirrorhelper", text = "Mirror Helper", icon_value=get_icon_id("MHelper"))
            if any("AutoMirror" in s for s in bpy.context.user_preferences.addons.keys()):
                layout.menu("automirror.submenu", text = "Automirror(T)", icon_value=get_icon_id("MHelper"))

        layout.separator()

        if right_handed_enabled():
            layout.operator("view3d.symmetrize", text = "(X) - Symmetrize", icon_value=get_icon_id("Xslap")).symtype = "POSITIVE_X"
            layout.operator("view3d.symmetrize", text = "(Y) - Symmetrize", icon_value=get_icon_id("Yslap")).symtype = "POSITIVE_Y"
        else:
            layout.operator("view3d.symmetrize", text = "(X) - Symmetrize", icon_value=get_icon_id("Xslap")).symtype = "NEGATIVE_X"
            layout.operator("view3d.symmetrize", text = "(Y) - Symmetrize", icon_value=get_icon_id("Yslap")).symtype = "NEGATIVE_Y"
        #layout.operator("view3d.symmetrize", text = "(Y) - Symmetrize", icon_value=get_icon_id("Yslap")).symtype = "NEGATIVE_Y"
        layout.operator("view3d.symmetrize", text = "(Z) - Symmetrize", icon_value=get_icon_id("Zslap")).symtype = "POSITIVE_Z"
        ...
        layout.operator("clean.recenter", text = "(S) Clean Recenter", icon_value=get_icon_id("SCleanRecenter"))
...
class eMeshtools(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        if pro_mode_enabled():    
            layout.menu("view3d.ewiz_submenu", text = "(E)Wizard")
