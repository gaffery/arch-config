...
class AdjustBevelOperator(bpy.types.Operator):
    bl_idname = "hops.adjust_bevel"
    bl_label = "Adjust Bevel"
    bl_options = {"REGISTER", "UNDO", "GRAB_CURSOR", "BLOCKING"}
    bl_description = "Change the bevel modifier of the active object"
    ...
    def draw(self, context):
        ...
        draw_text("B-Width - {:.3f} // (W) - {}".format(bevel.width, bevel.offset_type),
    ...
    def get_description_text(self):
        if self.object.hops.is_csharp:
            return "CSsharp / Ssharp"
        if self.object.hops.is_cstep:
